...
if ($exist && mysqli_num_rows($exist) > 0) {
 ...
 if (mysqli_num_rows($exist) > 0) {
    echo "<script>
            alert('Already registered for tabletennis');
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Player 2 information (if applicable)
    // For player 2 fields, if empty string, set to NULL
...
$play_style2 = !empty($_POST['play_style2']) ? mysqli_real_escape_string($con, $_POST['play_style2']) : null;
    ...
    // Determine role value based on original role input
    $role = ($role === "team") ? "double" : "single";

    // Insert query
    $query = "INSERT INTO tabletennis_players 
(fullName, email, role, teamName, hand1, play_style1,
 ...
 allergies2, medications2, surgeries2, previous_injuries2, hand2, play_style2)
VALUES (
    '$fullName', 
    '$email', 
    '$role', 
...
<script>
    function togglePlayerFields() {
      ...
      if (role === "team") {
        ...
        document.getElementById("hand2").required = true;
        document.getElementById("play_style2").required = true;
        ...
        document.getElementById("hand2").required = false;
        document.getElementById("play_style2").required = false;
    ...
    function validateForm(event) {
      ...
      // Player 1 table tennis specific validation
      const hand1 = document.getElementById("hand1").value;
      if (!hand1) {
        alert("Please select playing hand");
        isValid = false;
      }
      
      const play_style1 = document.getElementById("play_style1").value;
      if (!play_style1) {
        alert("Please select play style");
        isValid = false;
      }
      ...
      if (role === "team") {
        ...
        // Player 2 table tennis specific validation
        ...
        const play_style2 = document.getElementById("play_style2").value;
        if (!play_style2) {
          alert("Please select play style");
          isValid = false;
        }
