...
if ($event_id <= 0) {
    ...
    $res = $con->query("SELECT id FROM events WHERE sport LIKE '%volleyball%' LIMIT 1");
    if ($res && $res->num_rows) {
        $row = $res->fetch_assoc();
...
// --- Fetch registered volleyball teams based on game name ---
$registered_players = [];
$registered_rows = [];

// Make sure $gameName is set (e.g., "VolleyBall")
$gameName = "VolleyBall"; // or from URL/form: $_GET['game'] ?? 'VolleyBall';

$pstmt = $con->prepare("SELECT team_name, captain_name, captain_position
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_match') {
    ...
    if ($mid > 0) {
        ...
        // stay on same page
        header("Location: volleyball_scheduling.php?event_id={$event_id}");
        exit();
    }
}
    ...
    if ($round === 1) {
        ...
        foreach ($team_names as $nm) {
            if (!in_array($nm, $registered_players)) {
                $errors[] = "team '{$nm}' is not registered for volleyball.";
            }
        }
    } else {
...
<head>
  <meta charset="utf-8">
  <title>VolleyBall Scheduling</title>
...
<div class="container">
  <div class="d-flex align-items-center mb-3">
    <div class="logo me-3">VBS</div>
    <div>
      <h3 style="margin:0">VolleyBall — Match Scheduling</h3>
