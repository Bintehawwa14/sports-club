...
// event_id: prefer GET ?event_id=... else choose first active event with sport containing 'Badminton' or id=1 fallback
$event_id = isset($_GET['event_id']) ? intval($_GET['event_id']) : 0;
if ($event_id <= 0) {
    // try to pick an event which includes 'Badminton' in sport or pick first event
    $res = $con->query("SELECT id FROM events WHERE sport LIKE '%Badminton%' LIMIT 1");
    if ($res && $res->num_rows) {
        $row = $res->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_match') {
    ...
    if ($mid > 0) {
        ...
        // stay on same page
        header("Location: badminton_scheduling.php?event_id={$event_id}");
        exit();
    }
}
...
$mdate = isset($mrow['match_date']) ? $mrow['match_date'] : null;
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'set_result') {
    ...
    if ($match_id <= 0 || $winner === '') {
        ...
        if (!$mr || $mr->num_rows === 0) {
            ...
            if (empty($mdate)) {
                $errors[] = "Match date is not scheduled yet. Please set match date before saving result.";
                ...
                $errors[] = "Invalid winner selected for this match.";
            } else {
                ...
                $messages[] = "Result saved successfully.";
                header("Location: badminton_scheduling.php?event_id={$event_id}");
                exit();
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'schedule_matches') {
    ...
    // Round 1 constraints: min 2(power of two not strictly required, BYE used)
   if ($round === 1) {
        $min = 2;
...
$ins = $con->prepare("INSERT INTO matches 
    (event_id, game, round, team1_name, team2_name, match_status, match_date) 
    VALUES (?, ?, ?, ?, ?, 'Scheduled', ?)");
...
<head>
  <meta charset="utf-8">
  <title>Badminton Scheduling</title>
...
<div class="container">
  <div class="d-flex align-items-center mb-3">
    <div class="logo me-3">BS</div>
    <div>
      <h3 style="margin:0">Badminton — Match Scheduling</h3>
...
<script>
  ...
  // Show name fields on page load if First Round is selected
  if (roundSelect.value === '1') {
  ...
  // when num changes (for First Round only)
  numInput.addEventListener('input', function(){
    ...
    if (r === 1) {
      ...
      this.value = n;
      renderNameInputs(n, r); // <-- Always render fields when number changes
    }
  });
  ...
  document.getElementById('scheduleForm').addEventListener('submit', function(e){
    ...
    if (r === 1) {
      if (inputs.length < firstRoundMin) { alert('First Round requires at least '+firstRoundMin+' players'); e.preventDefault(); return;}
