...
if ($event_id <= 0) {
    // try to pick an event which includes 'tabletennis' in sport or pick first event
    $res = $con->query("SELECT id FROM events WHERE sport LIKE '%tabletennis%' LIMIT 1");
    if ($res && $res->num_rows) {
        $row = $res->fetch_assoc();
...
// --- Fetch registered badminton players (only players role) ---
$registered_players = [];
$registered_rows = [];
$pstmt = $con->prepare("SELECT id, fullName, email, role FROM tabletennis_players WHERE game = ? ORDER BY fullName ASC");
$pstmt->bind_param("s", $gameName);
$pstmt->execute();
$pres = $pstmt->get_result();
while ($r = $pres->fetch_assoc()) {
    $registered_rows[] = $r;
    $registered_players[] = $r['fullName'];
}
$pstmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_match') {
    ...
    if ($mid > 0) {
        ...
        // stay on same page
        header("Location: tabletennis_scheduling.php?event_id={$event_id}");
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'schedule_matches') {
    ...
    foreach ($team_names as $playerName) {
        ...
        if ($role) {
            ...
            $errors[] = "Player '{$playerName}' role not same.";
        }
    }
    ...
    if ($round === 1) {
        ...
        foreach ($team_names as $nm) {
            if (!in_array($nm, $registered_players)) {
                $errors[] = "Player '{$nm}' is not registered for tabletennis.";
            }
        }
    } else {
    ...
    if (empty($errors)) {
        ...
        $ins = $con->prepare("INSERT INTO matches (event_id, game, round, team1_name, team2_name, match_status, match_date) 
VALUES (?, ?, ?, ?, ?, 'Scheduled', ?)");

foreach ($pairs as $p) {
    $ins->bind_param("isisss", $event_id, $gameName, $round, $p[0], $p[1], $match_date);
    $ins->execute();
        }
        $ins->close();
...
<head>
  <meta charset="utf-8">
  <title>Table Tennis Scheduling</title>
...
<div class="container">
  <div class="d-flex align-items-center mb-3">
    <div class="logo me-3">TTS</div>
    <div>
      <h3 style="margin:0">Table Tennis — Match Scheduling</h3>
  ...
  <div class="card mb-4">
    <div class="d-flex justify-content-between align-items-center mb-2">
      <div>
        ...
        <div class="form-note">Showing players registered for Table Tennis. Use these exact names when scheduling.</div>
      </div>
      <div>
        <a href="tabletennis_players.php" class="btn btn-outline-secondary">Back to Registrations</a>
      </div>
    </div>
