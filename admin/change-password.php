...
if (isset($_POST['update'])) {
    $oldpassword = $_POST['currentpassword'];  // No need to hash here for comparison
    $newpassword = $_POST['newpassword'];

    $userid = $_SESSION['adminid'];

    // Fetch stored password from the database
    $sql = mysqli_query($con, "SELECT password FROM users WHERE id='$adminid'");
    $admin = mysqli_fetch_assoc($sql);

    if (password_verify($oldpassword, $user['password'])) {  // Use password_verify()
        $newpassword_hash = password_hash($newpassword, PASSWORD_DEFAULT);
        $ret = mysqli_query($con, "UPDATE users SET password='$newpassword_hash' WHERE id='$adminid'");
