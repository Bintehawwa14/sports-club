...
// Full Double-Elimination scheduling with UI matching badminton_scheduling.php
...
function getPlayerRole(mysqli $con, string $game, string $playerName) : ?string {
    $stmt = $con->prepare("SELECT role FROM tabletennis_players WHERE game = ? AND fullName = ?");
...
// Resolve BYE matches
function resolveByeIfAny(mysqli $con, int $match_id) {
    ...
    if ($t1 !== '' && ($t2 === '' || strtoupper($t2) === 'BYE')) {
        ...
        promoteAfterResult($con, $event_id ?? 0, $row['game'], $matchRow);
    }
}
...
$pr = $con->prepare("SELECT id, fullName, email, role FROM tabletennis_players WHERE game=? ORDER BY fullName ASC");
...
// Validate roles - all players must have the same role
    ...
    if (empty($errors)) {
        ...
        header("Location: tabletennis_double.php?event_id={$event_id}");
        exit();
    }
}
...
<head>
  <meta charset="utf-8">
  <title>Table Tennis — Double Elimination Scheduling</title>
...
<div class="container">
  <div class="d-flex align-items-center mb-3">
    <div class="logo me-3">TTD</div>
    <div>
      <h3 style="margin:0">Table Tennis — Double Elimination Scheduling</h3>
  ...
  function renderNameInputs(n, bracket, round) {
    ...
    for (let i = 1; i <= n; i++) {
      ...
      input.addEventListener('blur', function(e) {
        ...
        } else if (bracket === '<?= BRACKET_WB ?>' && round > 1) {
          const prevWinners = winnersByRound.WB[round - 1] || [];
          if (!prevWinners.includes(val)) {
            alert('"' + val + '" is not a winner from the previous round. Please choose from allowed list.');
            e.target.value = '';
            e.target.focus();
            return;
          }
